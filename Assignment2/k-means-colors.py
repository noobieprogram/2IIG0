...
def k_means(r: int, D: np.ndarray, init: str, dist: str):
    ...
    plt.savefig("initial{}.png".format(name))
    old_centroids = None
    iterations = 0
    while True:
...
def main():
    ...
    plt.scatter(Dx, Dy, c=label_color)
    name = "-{}-{}".format(init, dist)
    plt.savefig("raw{}.png".format(name))
    ...
    plt.clf()
    label_color = [LABEL_COLOR_MAP[l] for l in Y]
    plt.scatter(Xx, Xy, c=label_color)
    plt.savefig("centroids{}.png".format(name))
